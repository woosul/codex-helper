...
`parallel-review` and `feature-delivery` are local adaptations of the official Subagents guidance, designed for Codex's native agent, profile, sandbox, and skill surfaces. `dual-loop-review` applies the inner/outer-loop and fresh-context ideas described above. No third-party source code is vendored. The manifest records upstream references, `local-adaptation` license status, and the last review date so later maintenance can identify provenance and staleness.
